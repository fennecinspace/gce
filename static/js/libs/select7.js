function remove_selected_item(elem, e) {
    ...
    if (selected_items.length == 1) {
        ...
        // enabling other selections
        var selections = Array.from(document.querySelector('#annonce_create').querySelectorAll('select'));
        var elem_selection = elem.parentElement.parentElement.parentElement.querySelector('select');
        selections = selections.filter(x => x != elem_selection);
        for (let i = 0; i < selections.length; i++) {
            selections[i].removeAttribute('disabled');
            selections[i].parentElement.querySelector('.select7_placeholder').innerHTML = selections[i].dataset.fillerText;
        }
    }
...
function add_selected_item(elem, e) {
    ...
    // disabling other selections
    var selections = Array.from(document.querySelector('#annonce_create').querySelectorAll('select'));
    selections = selections.filter(x => x != elem);
    for (let i = 0; i < selections.length; i++) {
        selections[i].setAttribute('disabled',true);
        selections[i].parentElement.querySelector('.select7_placeholder').innerHTML = 'Désactiver';
    }
}
